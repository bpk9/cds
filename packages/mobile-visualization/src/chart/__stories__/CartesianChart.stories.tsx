...
import { TextHeadline } from '@coinbase/cds-mobile/typography';
...
import { GradientLine, Line, ReferenceLine, SolidLine, type SolidLineProps } from '../line';
...
const PredictionRow = memo(
  ({ seriesData, currentPrice, isSelected, onSelect, controlColor }: PredictionRowProps) => (
    <Pressable alignItems="center" gap={3} justifyContent="space-between" onPress={onSelect}>
...
const PredictionMarket = () => {
  ...
  const seriesConfig = useMemo(
    ...
    [eaglesData, theme.color.accentBoldBlue, theme.color.accentBoldGreen],
  );
  ...
  return (
    <VStack gap={4}>
      <VStack paddingTop={2}>
        <Text font="title1">Super Bowl LX</Text>
      ...
      <VStack gap={3}>
        <HStack alignItems="center" gap={2}>
          <Text font="title3">Make a prediction</Text>
        </HStack>
        <VStack gap={2}>
          {seriesConfig.map((series) => (
            <PredictionRow
...
const EarningsHistory = () => {
  ...
  const surprisePercentage = useCallback(
    (index: number): ChartTextChildren => {
      ...
      // Return plain string since mobile doesn't support styled TSpan in tick labels
      return `${percentage > 0 ? '+' : ''}${percentageString}`;
    },
  ...
  const LegendDot = memo(({ opacity = 1 }: { opacity?: number }) => {
    return (
      <Box background="bgPositive" borderRadius={1000} height={10} opacity={opacity} width={10} />
    );
  });
...
export const Miscellaneous = ChartStories;
