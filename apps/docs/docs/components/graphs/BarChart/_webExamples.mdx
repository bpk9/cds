...
function PositiveAndNegativeCashFlow() {
  ...
  const formatValue = useCallback(
    (value: number) => `${value < 0 ? '-' : ''}$${Math.abs(value)}M`,
    [],
  );

  const categories = Array.from({ length: 31 }, (_, i) => `3/${i + 1}`);
  const gains = [
    5, null, 6, 18, 0, 5, 12, 0, 12, 22, 28, 18, 0, 12, 6, 0, 0, 24, 0, 0, 4, 0, 18, 0, 0, 14, 10, 16,
    0, 0, 0,
  ];

  const losses = [
    -4, null, -8, -12, -6, 0, 0, 0, -18, 0, -12, 0, -9, -6, 0, 0, 0, 0, -22, -8, 0, 0, -10, -14, 0, 0,
    0, 0, 0, -12, -10,
  ];
  const series = [
    { id: 'Gains', data: gains, color: 'var(--color-fgPositive)', legendIndicator: 'pill' },
    { id: 'Losses', data: losses, color: 'var(--color-fgNegative)', legendIndicator: 'pill' },
  ];
  ...
  return (
    ...
    >
      <ChartTooltip label={(index) => categories[index] ?? '--'} valueFormatter={formatValue} />
    </BarChart>
  );
}
```
