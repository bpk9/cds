...
function Candlesticks() {
  ...
  const stockData = useMemo(() => btcCandles.slice(0, 90)
    .reverse(), []);
  const min = useMemo(() => Math.min(...stockData.map((data) => parseFloat(data.low))), [stockData]);

  const ThinSolidLine = memo((props) => <SolidLine {...props} strokeWidth={1} />);

  const candlesData = useMemo(() => stockData.map((data) => [parseFloat(data.low), parseFloat(data.high)]), [stockData]);
