...
Use the `flexDirection` prop to arrange legend items in a row or column. Note that when using `legend` as a boolean with `legendPosition`, the direction is automatically set based on position.
