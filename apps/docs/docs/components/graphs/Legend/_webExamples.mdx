...
function InteractiveLegend() {
  ...
  return (
    <VStack gap={2}>
      ...
      >
        <Legend position="top" ItemComponent={ChipLegendItem} gap={1} paddingTop={1} />
      </LineChart>
    </VStack>
  );
}
...
You can use `IndicatorComponent` to customize how the legend indicator is rendered while keeping the default item layout. This is useful when you want a custom visual style for the indicator without rebuilding the entire legend item.
