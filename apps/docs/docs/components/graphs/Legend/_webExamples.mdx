...
function CustomLegendWithValues() {
  ...
  return (
    <VStack gap={2}>
      ...
      >
        <Scrubber hideBeaconLabels />
        <Legend position="top" ItemComponent={ValueLegendItem} justifyContent="flex-start" />
      </LineChart>
    </VStack>
  );
}
...
function InteractiveLegend() {
  ...
  const ChipLegendItem = memo(({ seriesId, label, color }: LegendItemProps) => {
    ...
    console.log('emphasizedId', isEmphasized);
  ...
  return (
    <VStack gap={2}>
      ...
      >
        <Legend position="top" ItemComponent={ChipLegendItem} gap={1} />
      </LineChart>
    </VStack>
  );
}
...
The `IndicatorComponent` prop allows you to customize how the legend indicator is rendered while keeping the default item layout. This is useful when you want a custom visual style for the indicator without rebuilding the entire legend item.

In this example, we create an indicator with a semi-transparent fill and a solid border to differentiate projected data from actual data.

```jsx live
function CustomIndicatorExample() {
  const months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun'];
  const actualData = [120, 150, 180, 210, 240, null];
  const projectedData = [null, null, null, null, null, 270];

  // Custom indicator component that renders a squircle with 50% fill opacity and solid border
  const OutlinedIndicator = memo(({ color, indicator }: LegendIndicatorProps) => {
    // If custom ReactNode is passed, render it directly
  ...
  return (
    <VStack gap={2}>
      ...
      >
        ...
        />
        <BarPlot />
        <Legend position="top" IndicatorComponent={OutlinedIndicator} seriesIds={['projected']} />
        <Legend position="top" seriesIds={['actual']} />
      </CartesianChart>
    </VStack>
  );
}
...
Series can use different indicators via the `legendIndicator` property. Available presets are `pill`, `circle`, `square`, and `squircle`. This is useful for distinguishing between different data types, such as actual vs. forecasted values.
      ...
      <CartesianChart
        ...
        series={[
          ...
          },
          {
            ...
            color: 'var(--color-fgPositive)',
            legendIndicator: DottedLegendShape,
            stackId: 'revenue',
