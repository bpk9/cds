...
function RevenueForecast() {
  ...
  // Custom bar component that renders forecast bars with dotted pattern fill
  const ForecastBarComponent = memo((props) => {
    const { seriesId, dataX } = props;
    const isForecast = seriesId === 'forecast';

    if (true) {
      // Render bar with dotted pattern mask for forecast
      const uniqueMaskId = `${maskId}-${dataX}`;
      return (
        <g mask={`url(#${uniqueMaskId})`}>
          <defs>
            <mask id={uniqueMaskId}>
              <DefaultBar {...props} fill={`url(#${patternId})`} stroke={props.fill} strokeWidth={2} />
            </mask>
          </defs>
          <DefaultBar {...props} />
        </g>
    ...
    // Render default bar for actual data
  ...
  return (
    <VStack gap={2}>
      ...
      >
        {/* Pattern definition for dotted forecast bars */}
        <defs>
          <pattern
            id={patternId}
        ...
        <YAxis
          ...
          tickLabelFormatter={numberFormatter}
        />
        <BarPlot BarComponent={ForecastBarComponent} />
        <Legend position="top" width="100%" />
      </CartesianChart>
    </VStack>
  );
}
