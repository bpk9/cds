...
function GainLossChart() {
  ...
  return (
    ...
    >
      ...
      <ScrubberLine hideOverlay />
      <ScrubberHead
        seriesId="prices"
        {...(highlightedItem === null ? { x: data.length - 1, y: data[data.length - 1] } : {})}
      />
    </Chart>
  );
}
...
function ForecastChart() {
  ...
  const generateMonthlyBitcoinData = () => {
    ...
    while (currentDate < endDate) {
      ...
      const monthlyChange = (Math.random() - 0.5) * 0.4; // ±20% monthly volatility
      const trendFactor =
    ...
    >
      <HStack gap={2} alignItems="center">
        ...
        <VStack gap={0.25} flexGrow={1}>
          <Text font="title1" color="bg">BTC</Text>
          <Text font="label1" color="fgMuted">Bitcoin</Text>
        </VStack>
        <VStack gap={0.25} alignItems="flex-end">
          <Text font="title1" color="bg">{formatPrice(latestPrice)}</TextLabel2>
          <Text font="label1" color="fgPositive">+{formatPercentChange(percentChange)}</TextLabel2>
        </VStack>
      </HStack>
      <div
       style={{  marginLeft: 'calc(-1 * var(--space-2))',
