...
You can configure chart transitions using the `transitionConfigs` prop. This example showcases stacked areas with animated data updates.
...
function AnimatedStackedAreas() {
  ...
  function generateNextValue(previousValue) {
    ...
    newValue = Math.max(0, Math.min(maxDataOffset, newValue));
    return newValue;
  }
  ...
  function generateInitialData() {
    ...
    let previousValue = Math.random() * maxDataOffset;
