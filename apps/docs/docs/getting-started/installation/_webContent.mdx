...
CDS requires the following peer dependencies:
- `react@^18.3.1`
- `react-dom@^18.3.1`
...
```bash
yarn add react@^18 react-dom@^18 framer-motion@^10
```

:::note
Most React projects already have `react` and `react-dom` installed. You may only need to install `framer-motion`.
:::
